...
import com.iqos.imageselector.adapter.FolderAdapter;
import com.iqos.imageselector.adapter.ImageAdapter;
import com.iqos.imageselector.entry.Folder;
import com.iqos.imageselector.entry.Image;
import com.iqos.imageselector.model.ImageModel;
import com.iqos.imageselector.utils.DateUtils;
...
public class ImageSelectorActivity extends AppCompatActivity {
    ...
    private void setFolder(Folder folder) {
        if (null != folder && null != mAdapter && !folder.equals(mFolder)) {
            mFolder = folder;
    ...
    private void setSelectImageCount(int count) {
        if (count == 0) {
            ...
            tvPreview.setText(String.format("预览(%s)", count));
            if (isSingle) {
                tvConfirm.setText("确定");
            } else if (mMaxCount > 0) {
                tvConfirm.setText(String.format("确定(%s/%s)", count, mMaxCount));
            } else {
                tvConfirm.setText(String.format("确定(%s)", count));
            }
        }
    }
